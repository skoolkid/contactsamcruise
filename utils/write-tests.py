...
def write_tests(superclass_name, class_name, options_list, games):
    print('from csctest import {}'.format(superclass_name))
    print('')
...
class_name = 'ContactSamCruise{}Test'.format(test_type.capitalize())
if test_type == 'asm':
    games = ['csc' + suffix for suffix in ('', '_load', '_save', '_start')]
else:
    games = ['csc']
