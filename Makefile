...
usage:
	...
	@echo "  csc      build the Contact Sam Cruise HTML disassembly"
	@echo "  csc.asm  build the Contact Sam Cruise ASM disassembly"
...
.PHONY: csc
csc:
	utils/csc2html.py $(HTML_OPTIONS)

.PHONY: csc.asm
csc.asm:
	mkdir -p $(BUILD)/asm
	utils/csc2asm.py $(ASM_OPTIONS) > $(BUILD)/asm/csc.asm
